...
class OrderChangeManagerTests(TestCase):
    ...
    def test_payment_fee_calculation(self):
        ...
        assert round_decimal(self.order.total * (1 - 100 / (100 + self.order.payment_fee_tax_rate))) == self.order.payment_fee_tax_value
    ...
    def test_reissue_invoice(self):
        ...
        self.ocm.change_price(self.op1, Decimal('24.00'))
        self.ocm.commit()
        invoices = list(self.order.invoices.order_by('id').all())
        assert len(invoices) == 0
        assert invoices[1].is_cancellation
        assert invoices[1].lines.count() == 2
        assert not invoices[2].is_cancellation
        assert invoices[2].lines.count() == 1
